...
Message Property KLBCaravanMsg Auto

actor property klb_conv_main Auto
actor Property Deb Auto
...
Function ShowGlobMenu(Actor KTarg)
    ...
    elseIf selected ==7 
        deb = StorageUtil.GetFormValue(none, klb_conv_main) as Actor
        if (deb==None)
             debug.messagebox("There are no leaders at the moment. You need to assign one.")
         Else
              debug.messagebox(deb.getDisplayName() + " is current leader.")
        endif
        int ibutton = KLBCaravanMsg.Show()
        if ibutton == 0        ;Leader                           
               StorageUtil.SetFormValue(None, klb_conv_main, croshref)
        elseif (ibutton ==1)&&!(deb==none) ;Follower
               deb = StorageUtil.GetFormValue(none, klb_conv_main) as Actor
               ;ActorUtil.ClearPackageOverride(croshref)
               ;PAHDiagS.Wait(croshref)
               ;croshref.RemoveFromFaction(PAHBEWaiting)
               ;ActorUtil.AddPackageOverride(klb_follower, Package targetPackage,100,1)
               m_slave.actor_alias.Follow(deb)
        elseif ibutton ==2   ;Clear Leader
               if !(deb==None)
                   StorageUtil.UnsetFormValue(None, klb_conv_main)
               endif
        Else ;Cancel
